...
const Productcard = ({image,price,title,rating}) => {
    ...
    return (
        <div>
            <Box bg={"whitesmoke"} borderRadius={"10px"} h={"68vh"} boxShadow={"rgba(99, 99, 99, 0.2) 0px 2px 8px 0px;"}  padding={"1rem"}>
              <Image h={"35vh"} w={"100%"} src={image}  alt={title}/>
              <Text  pt={"1rem"} pl={"1.1rem"} fontWeight={"bold"} fontSize={"1rem"} color={"grey"}>{title}</Text>
              <Flex>
              <Text  color={"black"} pt={"0.6rem"} pl={"1.3rem"} fontSize={"1.3rem"} fontWeight={"bold"} c={"black"}>₹{price} </Text>
